...
\begin{tikzpicture}[node distance=2mm]
  ...
  \node[isbox, fill=isPLC!30, draw=isPLC, minimum width=70mm, minimum height=8mm, above=of cotp] (s7p) {S7commPlus (Session-keyed)};
\end{tikzpicture}
\vspace{2mm}
...
\begin{tikzpicture}[x=11mm, y=8mm, font=\scriptsize]
  \foreach \x/\w/\name/\col in {0/1/{0x72}/isOT, 1/1/{Ver}/isOT, 2/2/{Len}/isOT, 4/3/{Cmd hdr}/isPLC, 7/4/{Encrypted data}/isAttack}{
...
\begin{itemize}
  ...
  \item Missing nonce checks in challenge-response auth (S7commPlus pre-2019).
...
Span-port IDS gives full visibility on legacy plaintext stacks and only partial visibility on the modern session-keyed stacks. Plan host-side telemetry for the latter.
\end{notebox}
\end{frame}
...
\referencesframe{
  ...
  \item IEC 62351 (multipart). \emph{Security for IEC TC 57 protocols}.
  ...
  \item Wireshark Foundation. ICS protocol dissector documentation.
  \item Scapy contributors. \emph{Scapy User Manual}, ICS protocol modules.
