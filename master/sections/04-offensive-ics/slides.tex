...
\begin{tikzpicture}[node distance=8mm]
  \node[isbox, fill=isNet!15, draw=isNet, minimum width=42mm, font=\small, align=center] (auth) {Authorised\\penetration test};
  \node[isbox, fill=isAccent!15, draw=isAccent, minimum width=42mm, font=\small, align=center, right=of auth] (red) {Authorised red team\\(internal-blind)};
  \node[isbox, fill=isAttack!15, draw=isAttack, minimum width=42mm, font=\small, align=center, right=of red] (unauth) {Unauthorised access\\(criminal)};
\end{tikzpicture}
\vspace{2mm}
\begin{importantbox}{Paper, not handshake}
``Verbal'' permission is no permission. A signed \emph{Rules of Engagement} document with named approvers, scope, timing, kill-switch, and reporting path is the gating artefact.
\end{importantbox}
...
Many vendors donate retired equipment to academic and CERT labs in exchange for confidentiality on findings. Worth asking.
\end{notebox}
\end{frame}
...
\centering
\begin{tikzpicture}[node distance=4mm]
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=46mm, font=\small] (a) {Metasploit (modbusclient, modicon\_stux)};
  \node[isbox, fill=isOT!15, draw=isOT, right=of a, minimum width=46mm, font=\small] (b) {Caldera + ATT\&CK ICS adversary emulation};
  \node[isbox, fill=isAccent!15, draw=isAccent, below=of a, minimum width=46mm, font=\small] (c) {Smod (Modbus pentest framework)};
  \node[isbox, fill=isAccent!15, draw=isAccent, right=of c, minimum width=46mm, font=\small] (d) {plcscan, pymodbus, snap7};
  \node[isbox, fill=isPLC!15, draw=isPLC, below=of c, minimum width=46mm, font=\small] (e) {ISF (Industrial Security Framework)};
  \node[isbox, fill=isPLC!15, draw=isPLC, right=of e, minimum width=46mm, font=\small] (f) {OpenVAS / Greenbone with ICS plug-ins};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=2mm]
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=110mm, font=\small] (a) {1.\ Executive summary -- 1 page, plain language};
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=110mm, font=\small, below=of a] (b) {2.\ Scope, ROE, attestations};
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=110mm, font=\small, below=of b] (c) {3.\ Findings, each: title, CVSS-E, evidence, repro};
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=110mm, font=\small, below=of c] (d) {4.\ Recommendations, sized this-quarter / this-year};
  \node[isbox, fill=isOT!15, draw=isOT, minimum width=110mm, font=\small, below=of d] (e) {5.\ Appendix: PCAPs, configs, raw logs};
\end{tikzpicture}
\vspace{1mm}
