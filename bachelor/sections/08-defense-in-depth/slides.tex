...
\begin{tikzpicture}[node distance=10mm]
  ...
  \draw[-{Stealth[length=2mm]}, dashed, thick] (sw) -- node[midway, right=3pt, font=\tiny, fill=white, inner sep=1pt]{mirror only} (ids);
  \draw[flow] (ids) -- (soc);
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=8mm]
  ...
  \draw[flow] (usb) -- (kiosk);
  \draw[flow] (kiosk) -- node[midway, above=4pt, font=\tiny, fill=white, inner sep=1pt]{cleared} (ws);
\end{tikzpicture}
\vspace{2mm}
