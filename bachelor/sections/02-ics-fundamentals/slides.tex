...
\end{notebox}
\source{ISA 5.1-2009 (Instrumentation Symbols) \textbar{} IEC 60381-1:1982 (DC analogue signals for process control, i.e. the 4-20\,mA loop).}
\note{
...
Stuxnet's payload commanded the \emph{VFDs} (variable-frequency drives) running the centrifuges from a compromised Siemens PLC --- the PLC was the foothold, but the final elements are where the physical consequence lived.
\end{notebox}
...
\begin{itemize}
  ...
  \item IEC 61131-3 languages:
    \begin{itemize}\scriptsize
      ...
      \item Instruction List \emph{(deprecated, 2013 ed.)}
    \end{itemize}
\end{itemize}
...
\end{tikzpicture}
\par\vspace{1mm}
\begin{notebox}{What it does (IT-bridge)}
\texttt{Start} energises the \texttt{Motor} coil. The seal-in branch around \texttt{Start} latches the coil so it stays on. \texttt{Stop} (NC) breaks both paths. \textbf{In IT terms:} \emph{coil} = output bit; \emph{NC contact} = inverted input; \emph{seal-in} = self-feedback latch (the classic SR-latch pattern).
\end{notebox}
\note{
...
\source{President's Commission on the Accident at Three Mile Island (Kemeny Commission), \emph{Report}, 1979.}
\note{
...
\begin{tikzpicture}[node distance=14mm]
  ...
  \node[plc, fill=isAttack!15, draw=isAttack, align=center, right=44mm of bpcs] (sis) {SIS\\(SIL-rated)};
...
\begin{tikzpicture}[x=2.4cm]
  \foreach \i/\code/\name/\col in {
    ...
    3/SFC/{Sequential}/isNet,
    4/IL/{IL (deprecated)}/isMute}{
...
\referencesframe{
  \item \emph{Cross-reference (deepened in Master):} Master \S\,01 (\emph{Firmware RE}) and \S\,02 (\emph{PLC Ladder RE}) deepen the controller-internals view.
