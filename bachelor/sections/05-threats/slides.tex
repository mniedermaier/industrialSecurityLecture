...
Four zero-days, two stolen code-signing certificates. Subtly destroyed $\sim$1\,000 centrifuges over months while feeding the HMI \emph{normal-looking} data. The first widely-known cyber-physical weapon.
\end{notebox}
\end{frame}
...
\end{tikzpicture}
\par\vspace{2mm}
\footnotesize First publicly known attack on a Safety Instrumented System. Programming error in the malware tripped the plant -- and exposed the operation. Watershed moment: attackers reached the safety layer.
\end{frame}
...
\end{tikzpicture}
\par\vspace{2mm}
\footnotesize IT compromise; operator pre-emptively halted OT. Highlights IT/OT interdependence and the physical impact of ransomware.
\end{frame}
